...
This programme simulates the motion of a system of particles, where particles 
interact via the Lennard-Jones potential:
    
U_ij=4*{1/r**12-1/r**6}, r=|ri-rj|
    
A second order Verlet time-integration loop with an appropriate timestep is used
to model the dynamics of the system. Applying pbc and mic, pair separations between
all the particles are generated as an NxNx3 array at every timestep in the integration.
This is used to compute forces, the potential energy and update particle velocities
and positions. The particles' mean square displacement and average radial distribution
function are also calculated.
    
The input file Ar(g/s)_input.txt and the names of the output files are given in
the command line. The programme plots the radial distribution function, mean square
displacement and the kinetic, potential and total energy of the system. Output files
for these quantities, as well as an XYZ trajectory file for all particles, are 
returned.
...
def scalar_seps(pair_seps, L):
    ...
    #the values in the lower left triangle will be the equal to their
    #respective transpose elements (pair_seps[x,y]=pair_seps[y,x])
    ...
    #Given forces is an antisymmetric matrix, only compute upper right triangle
        ...
        for y in range(x, len(pair_seps)):
            ...
            if 1e-15<r<3.5: 
                ...
                forces[y,x]=-1*forces[x,y] #Newton's third law
...
def MSD(particles, initial_pos, L):
    ...
    :param initial_pos: Nx3 numpy array containing initial positions of all 
    N particles
    :param L: scalar cubic side length of simulation unit cell
...
def RDF(ss, L, rho, numstep, particles):
    ...
    #convert list of bin edge points into halfway points between bin edges
    #this ensures the two returned lists are of equal size
    r=r_long[1:]-0.5*dr
...
def update_position(particles, force_array, dt, L):
    ...
    :param particles: numpy array of N Particle3D instances
    :param force_array: NxNx3 numpy array giving LJ forces acting on N particles
    :param dt: timestep used in integration
    :param L: cubic side length of unit cell used in simulation
...
def main():
    ...
    #set initial positions and velocities using md_utils programme
    ...
    # Read name of output XYZ file from command line
