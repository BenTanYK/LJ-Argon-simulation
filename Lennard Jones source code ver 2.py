...
def update_all_forces(particles):
    """
...
def main():
    ...
    # Initial conditions
    time=0
    forces=update_all_forces(particles)
    ...
    for i in range(numstep):
        ...
        # Update forces
        forces_new = update_all_forces(particles)
        ...
        energy=tot_potential_energy(particles)+Particle3D.sys_kinetic(particles)
...
p1=Particle3D('p1', 1, np.array([7+2**(1/6)+0.1,0,0]), np.array([0.01, 0, 0]))
p2=Particle3D('p2', 1, np.array([7,0,0]), np.array([-0.01, 0, 0]))
