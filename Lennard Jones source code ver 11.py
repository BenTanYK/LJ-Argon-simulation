...
def RDF(ss, L):
    ...
    :return: two lists containing values of separation r_ij and the frequency
    for the corresponding 
    """ 
    

    rdf, av_r=np.histogram(ss[ss>0], bins=100, range=(0,0.5*math.sqrt(3)*L))
...
def main():
    ...
    rdf, r_long=RDF(rdf_array, L)
    
    #remove last element of r_lon
    r=r_long[:-1]
    
    #empty list to hold normalised rdf values
    rdf_normalised_list=[]
    
    #divide each value of rdf by r**2 to normalise
    ...
    energy_file.write('Time '+'Kinetic energy '+'Potential energy '+'Total energy '+'\n')
